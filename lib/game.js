...
const gameCasinoSolo = async (conn, m, prefix, db) => {
        try {
                ...
                if (isNaN(m.args[0])) return m.reply(`Enter a valid amount!\nExample: ${prefix + m.command} 1000`)
                if (db.users[m.sender].money >= count * 1) {
                        ...
                        if (Aku > Kamu) {
                                ...
                                m.reply(`💰 Casino 💰\n*You:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*You Win*\nYou gained ${count * 2} Money`.trim())
                        } else {
                                db.users[m.sender].money += count * 1
                                m.reply(`💰 Casino 💰\n*You:* ${Kamu} Point\n*Computer:* ${Aku} Point\n\n*TIE*\nYou got back ${count * 1} Money`.trim())
                        }
                } else m.reply(`You don't have enough money for Casino. Please *collect* more first!`)
        } catch (e) {
                m.reply('Error!')
        }
}
...
const gameSamgongSolo = async (conn, m, db) => {
        ...
        if (db.users[m.sender].money < count) return m.reply(`You don't have enough money for Samgong. Please *collect* more first!`)
        db.users[m.sender].money -= count;
        db.users[m.sender].limit -= 1
        let { key } = await m.reply('*🃏Game started!* Cards are being dealt...');
        await sleep(5000);
        ...
        let result = playerScore > 30 ? '💀 You lose!' : playerScore === botScore ? '🤝 It\'s a Tie! Bet returned' : botScore > 30 || playerScore > botScore ? `🎉 You win! +${winnings} 💵` : '😞 Bot wins!';
...
const gameMerampok = async (m, db) => {
        ...
        if (new Date - db.users[m.sender].lastrampok > 3600000) {
                ...
                if (!who) return m.reply('Tag someone to rob!')
                if (!db.users[who]) return m.reply('Target is not registered in the database!')
                if (10000 > db.users[who].money) return m.reply('Target is broke 🗿')
                db.users[who].money -= dapat
                db.users[m.sender].money += dapat
                db.users[m.sender].lastrampok = new Date * 1
                m.reply(`Successfully robbed ${dapat} Money from the target!`)
        } else m.reply(`You already robbed recently. Wait ${timers} before robbing again.`)
}
...
const gameBegal = async (conn, m, db) => {
        ...
        let random = [
                {teks: 'Target Escaped!', no: 0},
                {teks: 'Target Ran Away!', no: 0},
                {teks: 'Target Hid Successfully', no: 0},
                {teks: 'Target Self-Destructed', no: 2},
                {teks: 'Target Was Caught', no: 2},
                {teks: 'Target Not Found!', no: 0},
                {teks: 'Target Was Stronger Than You!', no: 1},
                {teks: 'Target Used a Cheat', no: 1},
                {teks: 'Target Called the Police', no: 0},
                {teks: 'Target Got Caught!', no: 2},
                {teks: 'Target Surrendered', no: 2}
        ]
        let teksnya = await pickRandom(random);
        if (new Date - user.lastbegal > 3600000) {
                let { key } = await m.reply('Searching for a target...')
                await sleep(2000)
                if (teksnya.no === 0) {
                        await m.reply({ text: teksnya.teks, edit: key })
                        await m.reply('Failed to find a target. Please try again.')
                } else if (teksnya.no === 1) {
                        await m.reply({ text: teksnya.teks, edit: key })
                        await m.reply(`You were beaten by the target\nYou lost *${randomUang}* Money`)
                        db.users[m.sender].money -= randomUang
                        db.set[botNumber].money += randomUang * 1
                } else {
                        await m.reply({ text: teksnya.teks, edit: key })
                        await m.reply(`Successfully obtained *${randomUang}* Money!`)
                        db.users[m.sender].money += randomUang
                        db.users[m.sender].lastbegal = new Date * 1
                }
        } else m.reply(`Please wait *⏱️${timers}* before playing again.`)
}
...
const daily = async (m, db) => {
        ...
        } else m.reply(`Please wait *⏱️${timers}* before claiming again.`)
}

const buy = async (m, args, db) => {
        if (args[0] === 'limit') {
                if (!args[1]) return m.reply(`Enter an amount!\nExample: ${m.prefix + m.command} limit 10`);
                let count = parseInt(args[1])
                if (db.users[m.sender].money >= count * 500) {
                        db.users[m.sender].limit += count * 1
                        db.users[m.sender].money -= count * 500
                        m.reply(`Successfully purchased ${args[1] * 1} limit for ${args[1] * 500} Money.`);
                } else m.reply(`You don't have enough money to buy limit!\nYour balance: ${db.users[m.sender].money}\nCost for ${args[1]} limit: ${args[1] * 500}`);
        } else m.reply(`Limit Price: Amount x 500\n• 1 limit = 500\n• 2 limit = 1000\n\nExample: .buy limit 3`);
}
                ...
                if (db.users[who]) {
                        if (db.users[m.sender].limit >= count * 1) {
                                try {
                                        ...
                                        m.reply(`Successfully transferred ${count} limit to @${who.split('@')[0]}`)
                                } catch (e) {
                                        db.users[m.sender].limit += count * 1
                                        m.reply('Transfer Failed')
                                }
                        } else m.reply(`Not enough limit!\nYour remaining limit: *${db.users[m.sender].limit}*`)
                ...
                if (db.users[who]) {
                        if (db.users[m.sender].money >= count * 1) {
                                try {
                                        ...
                                        m.reply(`Successfully transferred ${count} Money to @${who.split('@')[0]}`)
                                } catch (e) {
                                        db.users[m.sender].money += count * 1
                                        m.reply('Transfer Failed')
                                }
                        } else m.reply(`Not enough money!\nYour remaining balance: *${db.users[m.sender].money}*`)
                } else m.reply(`Number ${who.split('@')[0]} is not a bot user!`)
...
class Blackjack {
        ...
        reuseSubmitCardsForDrinking() {
                ...
                if (this.submitCard.length === 1 && this.isRoundComplete()) {
                        ...
                        return { msg: `Only @${owner.split('@')[0]} has a card — they become the new leader.`, continue: true };
                }
                ...
                this.submitCard = [];
                return { msg: 'Cards distributed to players who must drink.', continue: true };
        }
}
